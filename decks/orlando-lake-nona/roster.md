...
## Slides 10–18 — "Example division funnel"

The nine funnel headings read **"Example division funnel"** rather than
"Division funnel". The label was added at the client's request so the room
takes the shape as an illustration rather than as a promise of what their
own child will get.

It is a presentation label, not a disclaimer about the data. The bars carry
the real, reconciled per-division counts, and every figure on these slides —
interested programs, profile views, the funnel itself — is shown in full and
traces to the rows recorded above. Do not read "example" as licence to put
invented numbers in those bars.

A blur over the interest and view figures was tried on 2 Sep 2026 and
reverted the same day at the client's request; the tiles show their real
numbers.
